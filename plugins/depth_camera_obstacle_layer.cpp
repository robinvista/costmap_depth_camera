...
void DepthCameraObstacleLayer::onInitialize()
{
  ...
  */
  frustum_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZI>>("frustum", 5);
  marking_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZI>>("marking_pc", 5);
  cluster_pub_ = nh.advertise<pcl::PointCloud<pcl::PointXYZI>>("clustered_pc", 5);
  ...
  */
  ...
  nh.param("transform_tolerance", transform_tolerance, 2.2);
  ...
  while (ss >> source)
  {
    ...
    source_node.param("min_obstacle_height", min_obstacle_height, -1.0);
    ...
    source_node.param("max_detect_distanxe", max_detect_distance, 5.5);
    ...
    boost::shared_ptr < message_filters::Subscriber<sensor_msgs::PointCloud2> > sub(new message_filters::Subscriber<sensor_msgs::PointCloud2>(g_nh, topic, 1000));
    ...
    boost::shared_ptr < tf::MessageFilter<sensor_msgs::PointCloud2> > filter(new tf::MessageFilter<sensor_msgs::PointCloud2>(*sub, *tf_, global_frame_, 1000));
    filter->registerCallback(boost::bind(&DepthCameraObstacleLayer::pointCloud2Callback, this, _1, observation_buffers_.back()));
...
void DepthCameraObstacleLayer::pointCloud2Callback(const sensor_msgs::PointCloud2ConstPtr& message, const boost::shared_ptr<ObservationBuffer>& buffer)
{
...
void DepthCameraObstacleLayer::ClearMarkingbyKdtree(pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_in,   std::vector<costmap_depth_camera::Observation>& observations,
                                          ...
                                          double* min_y, double* max_x, double* max_y)
{
  ...
  */
  ...
  ROS_INFO("point size before filtered: %d ", int(combined_observations->points.size()));
  ...
  ROS_INFO("point size  after filtered: %d ", int(combined_observations->points.size()));
  ...
  if(combined_observations->points.size()>size_of_cluster_rejection_){
